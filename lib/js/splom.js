/**
 * Scatter plot
 * Adapted from http://mbostock.github.com/d3/ex/splom.js
 */
var drawScatterplot = function(input) {
  ...
  */
  ...
  var svg = d3.select("#chart")
    .append("svg:svg")
      .attr("width", size * l_bins)
      .attr("height", size * input.length);

  // One row per category value
  var row = svg.selectAll("g")
      .data(input) //FIXME this will break later.
    .enter().append("svg:g")
      .attr("transform", function(d, i) { return "translate(0," + i * size + ")"; });

  // One column per l-bin.
  
  var column = row.selectAll("g")
      .data(cross(color_frequencies))
    .enter().append("svg:g")
      .attr("transform", function(d, i) { return "translate(" + i * size + ",0)"; });

  // X-ticks. TODO Cross the trait into the tick data?
  column.selectAll("line.x")
      .data(function(d) { return bScale.ticks(5).map(bScale); })
  ...
  /*
   * TODO Make this work for color data
   */
  var dot = column
   		.append("svg:circle")
	      .attr("cx", function(d,i) {  console.log("A = ", d); return aScale(d.y.results.values.lab.a); })
	      .attr("cy", function(d) { return bScale(d.y.results.values.lab.a); })
	      .attr("r", 3)
